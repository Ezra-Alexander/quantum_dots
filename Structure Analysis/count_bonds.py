...
for i, atom1 in enumerate(atoms):
	if index_in[i]:
		for j,atom2 in enumerate(atoms):
			if dists[i][j] > 2.45 and dists[i][j] < 3 and index_f[j]:
...
for i, atom in enumerate(atoms):
	if index_in[i]:
		...
		in_p_bonds[i] = 0
		for j,atom in enumerate(atoms):
			if dists[i][j] < 2.45 and index_f[j]:
				in_f_bonds[i]+=1
...
for i, atom in enumerate(atoms):
	if index_in[i]:
		indiums.append(str(in_p_bonds[i]+in_f_bonds[i])+" , "+str(in_f_bonds[i]))
...
for i,ind in enumerate(unique_in):
	...
	for j, atom in enumerate(indiums):
		if ind == indiums[j]:
			count += 1
	name = ind[:2] + "Coordinate Total" + ind[2:] + " F"
